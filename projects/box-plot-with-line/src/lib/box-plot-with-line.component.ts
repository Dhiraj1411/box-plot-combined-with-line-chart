...
export class BoxPlotWithLineComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    if (this.options.showBoxPlot) {
      this.drawBoxPlotChart(this.options.box_plot_data);
    }
    if (this.options.showLine) {
      this.drawLineChart(this.options.box_plot_data, this.options.currentLine, this.options.medianLine);
    }
  }

  validateInput = () => {
    // Validation for Box plot data
    if (this.options.showBoxPlot) {
      if (this.options.box_plot_data) {
        this.options.box_plot_data.forEach((data) => {
          let whiskers, quartile, key;
          [key, quartile, whiskers] = Object.keys(data);
          if (key !== 'key' || quartile !== 'quartile' || whiskers !== 'whiskers') {
            console.error(`key : ${key}, quartile : ${quartile}, whiskers : ${whiskers}`);
          }
        });
      } else {
        console.log('No Data for Box plot');
      }
    }
  }
  ...
  getYMaxMinValue = () => {
    ...
    this.options.box_plot_data.forEach((data) => {
      tempArray.push(...data['whiskers']);
    });

    this.options.currentLine.forEach((obj: Array<any>) => {
      tempArray.push(obj['y']);
    });

    this.options.medianLine.forEach((obj: Array<any>) => {
      tempArray.push(obj['y']);
    });
  ...
  drawBoxPlotChart = (BOX_PLOT_DATA) => {
    ...
    this.svg = d3.select('#chartDiv').append('svg').attr('viewBox', '0 0 1200 500')
      .append('g')
  ...
  drawLineChart = (BOX_PLOT_DATA, CURRENT_LINE, MEDIAN_LINE) => {
...
interface DataInput {
  box_plot_data: Array<any>;
  showLine: Boolean;
  showBoxPlot: Boolean;
  medianLine: Array<any>;
  currentLine: Array<any>;
  colorArrayForBoxPlot: Array<any>;
  colorArrayForLine: Array<any>;
}
